...
# ╔═╡ bbd0e8dd-d325-4449-927d-f9f73f217778
using AstroImages, Photometry, CoordinateTransformations, WGLMakie
...
# ╔═╡ 1baba9f9-3e9b-4f8f-81ab-7efca7cfc6c7
to_points = Point2.([[0, 1], [0, 0], [1, 0]])

# ╔═╡ 674ae975-48f3-4772-a0da-75e90b6d3e10
l = √(2) / 2

# ╔═╡ 0571ee2a-8c6c-4000-8dfb-c0f682014109
from_points = Point2.([[-l, l], [0, 0], [l, l]])

# ╔═╡ 0b381af7-b8f7-4fad-9ae4-09c72227355f
# let
# 	fig, ax, sc = scatter(from_points; axis=(; limits=(-2, 2, -2, 2)))
# 	scatter!(ax, to_points)
# 	fig
# end

# ╔═╡ 56ad9257-3d6b-4e39-ad3e-92c3c4c23da0
r2 = restrict ∘ restrict

# ╔═╡ bf66f454-4d5a-4d7d-898b-fae5b40cc090
...
# ╔═╡ 5ae9e703-7c83-41dd-a4c3-252972ef59aa
function apertures!(ax, aps; radius=12, color=:lightgreen)
	for ap in aps
		arc!(ax, Point2f(ap.x, ap.y), radius, 0, 2π; color)
	end
end
...
let
	fig = Figure()
	ax = Axis(fig[1, 1])
	sl = Makie.Slider(fig[2, 1]; range=1:2)
	
	img = @lift imgs[$(sl.value)]

	hm = heatmap!(ax, img)

	apertures!(ax, ap_sources1)
	apertures!(ax, ap_sources2; color=:red)
...
# ╔═╡ 91c1ac28-40e9-4667-8805-95605d145b22
WGLMakie.Page()

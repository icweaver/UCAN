...
# ╔═╡ 13eed506-1d32-47ad-b10c-4473f7983907
md"""
### hmmm
"""
...
function get_sources(img; box_size=(5, 5), nsigma=1)
	...
	bkg, bkg_rms = estimate_background(clipped, 8)
	subt = img .- bkg[axes(img)...]
	errs = ones(axes(subt)) .* bkg
	 # Sort from brightest to darkest
	return extract_sources(PeakMesh(; box_size, nsigma), subt, errs, true), subt, errs
end
...
sources_all, subt, errs = get_sources(img; nsigma=0.25) # sort from brightest to darkest

# ╔═╡ d65a28f9-04b5-44cc-9ffa-c304b2cd6fd0
dark = load("./mgcc3f_2024-03-25T07-10-03.022_DARKFRAMEMEAN.fits");
...
# ╔═╡ 8a85287c-8023-48e8-89cd-35ebee08220d
photometry(aps, subt)
...
phot1 = first(get_photometry(sources1, subt1), 8)

# ╔═╡ f2d3b19f-a265-4b42-af0b-ee78eb0346f2
# C1 = combinations(first(sources1, 10), 3)
C1 = combinations(first(phot1, 10), 3)
...
phot2 = first(get_photometry(sources2, subt2), 8)

# ╔═╡ f6060373-995c-4630-a253-cc794dd3c852
# C2 = combinations(first(sources2, 10), 3)
C2 = combinations(first(phot2, 10), 3)
...
let
	...
	p2 = scatter(x=first.(tri_invars2), y=last.(tri_invars2); mode=:markers, marker=attr(symbol="circle-open", size=10))
...
# ╔═╡ d639a572-f7b6-4878-a05c-ef1c634da8e3
# control_points = [
# 	6 0
# 	3 4
# 	0 0
# 	3 -3
# ]
control_points = last.(point_map)

# ╔═╡ 52a72ba1-4fc3-446e-9c0c-cf3af11c38b0
inv_uniq, triang_vrtx_uniqu = aa._generate_invariants(np.array(control_points));

# ╔═╡ 7385dff5-ed0c-4ca8-bb10-769ee52dd080
inv_uniq
...
# ╔═╡ c47f33f1-49b0-4ba8-8fa8-94d5964590a1
plot_pair(img1, img)
